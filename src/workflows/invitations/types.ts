...
export type PendingInvitationAction = { operation: InvitationOperation; invitationId?: string };
...
export type InvitationOutcome<T> =
  ...
  | { status: "error"; error: InvitationWorkflowError }
...
type RecipientActions<C extends OrganizationClient> = InvitationActionState & {
  ...
  reject: (
    invitationId: string,
  ) => Promise<InvitationOutcome<Data<C["organization"]["rejectInvitation"]>>>;
...
export type InvitationResponseState<C extends OrganizationClient> = Omit<
  ...
  "data"
> &
  Omit<RecipientActions<C>, "accept" | "reject" | "retrySync" | "pendingSync"> & {
...
export type InvitationFormOptions<C extends OrganizationClient> = {
  ...
  validate?: (
    ...
    | InvitationFieldErrors<InvitationValues<C>>
    | Promise<InvitationFieldErrors<InvitationValues<C>>>;
...
export interface InvitationFormComponent<C extends OrganizationClient> {
  (
    props: InvitationFormOptions<C> & { children: (state: InvitationFormState<C>) => ReactNode },
  ): ReactNode;
  <S extends InvitationSchema<C>>(
    props: SchemaInvitationFormOptions<C, S> & {
...
> &
  InvitationActionState & {
    cancel: (
      invitationId: string,
    ) => Promise<InvitationOutcome<Data<C["organization"]["cancelInvitation"]>>>;
    resend: (
      values: InvitationValues<C>,
    ) => Promise<InvitationOutcome<Data<C["organization"]["inviteMember"]>>>;
  };
