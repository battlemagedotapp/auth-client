...
export type InvitationResponseState<C extends OrganizationClient> = Omit<ReadState<Data<C["organization"]["getInvitation"]>>, "data"> & Omit<RecipientActions<C>, "invitation"> & {
...
export type InvitationFormOptions<C extends OrganizationClient> = WorkflowFeedbackOptions & {
    ...
    mode?: "invite" | "resend";
    enabled?: boolean;
    validate?: (values: Readonly<InvitationValues<C>>) => FormFieldErrors<InvitationValues<C>> | Promise<FormFieldErrors<InvitationValues<C>>>;
    onInvited?: Callback<Data<C["organization"]["inviteMember"]>>;
};
export type InvitationFormState<C extends OrganizationClient, V = InvitationValues<C>> = WorkflowForm<V, Data<C["organization"]["inviteMember"]>>;
...
export type InvitationFormComponent<C extends OrganizationClient> = (props: InvitationFormOptions<C> & {
    children?: ReactNode;
}) => ReactNode;
export type OrganizationInvitationsOptions<C extends OrganizationClient> = WorkflowFeedbackOptions & {
    organizationId: string;
    enabled?: boolean;
...
export type OrganizationInvitationsState<C extends OrganizationClient> = ReadState<Data<C["organization"]["listInvitations"]>> & WorkflowActionState & {
    invitation(invitationId: string): {
...
export type InvitationSurface<C extends OrganizationClient> = {
    ...
    defineInvitationForm<S extends InvitationSchema<C>>(schema: S & SchemaBoundary<S>): WorkflowDefinition<Omit<SchemaInvitationFormOptions<C, S>, "schema"> & {
        schema?: S & SchemaBoundary<S>;
